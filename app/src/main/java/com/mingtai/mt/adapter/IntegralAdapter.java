...
public class IntegralAdapter extends RecyclerView.Adapter<IntegralAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        if (balanceDetailBeans.get(position).getReceiptsOrOut() == -1) {
            ...
            holder.tv_price.setTextColor(mContext.getResources().getColor(R.color.black_title_text));
        } else {
            if (type == 1) {
                holder.tv_price.setText("+" + expenditureMoney + "元");
            } else {
...
//        if (type == 1){
        ...
        String balance = MingtaiUtil.isCoin(balanceDetailBeans.get(position).getBalance());
